...
class ZeroAgent(Agent):
    ...
    def _mcts(self, root_id):
        ...
        for i in range(num_mcts):
            ...
            value, reward = self._expansion_simulation(leaf_id, win_index)

            # backup
            self._backpropagation(leaf_id, value, reward)
    ...
    def _expansion_simulation(self, leaf_id, win_index):
        ...
        self.model.eval()
        with torch.no_grad():
            state_input = torch.tensor([leaf_state]).to(device).float() # k=leaf_state: 5x15x15 tensor
            policy, value = self.model(state_input)  # 훈련중 MCTS를 수행할 때 NN 사용
    ...
    def _backpropagation(self, leaf_id, value, reward):
        node_id = leaf_id
        count = 0
    ...
    def get_pv(self, root_id):
        ...
        with torch.no_grad():
            ...
            policy, value = self.model(state_input) # 훈련을 위한 용도 X
